...
namespace YnABMC
{
    ...
    public partial class Form1 : Form
    {
        ...
        string SoftwareVersion = "Alpha 0.3.4.1";
        //string SoftwareVersion = "Dev 0.3.4.1";
...
#region Methods
        ...
        {
            s = Regex.Replace(s, @"[ÅÆĀĂĄÀÁÂÃÄȀǞĄ̊ḀA͗Ⱥ]", "A");
            s = Regex.Replace(s, @"[ḂḄḆɃƁ]", "B");
            s = Regex.Replace(s, @"[ĆĈČÇḈĊƇȻ]", "C");
            s = Regex.Replace(s, @"[ƊḒĎḊḌḎḐĐÐƉḎ]", "D");
            s = Regex.Replace(s, @"[ÉÊÈȄĚËĒ‌‌ĔĘĖẸȆḔḖḘḚḜɆ€]", "E");
            s = Regex.Replace(s, @"[ḞƑ]", "F");
            s = Regex.Replace(s, @"[ǴĜǦĞĠĢƓḠǤ]", "G");
            s = Regex.Replace(s, @"[ĤȞḦḢḤH̱ḨḪĦⱧ#]", "H");
            s = Regex.Replace(s, @"[İIÍÎÌȈÏḮĪĨĮỊḬ]", "I");
            s = Regex.Replace(s, @"[ĴJ̌Ɉ]", "J");
            s = Regex.Replace(s, @"[ḰǨĶḲḴƘⱩ]", "K");
            s = Regex.Replace(s, @"[ĹŁḼĽĻḶḸḺȽⱢⱠ£]", "L");
            s = Regex.Replace(s, @"[ḾṀṂ]", "M");
            s = Regex.Replace(s, @"[ŊŃǸÑṊŇṄṆṈN̈Ņ]", "N");
            s = Regex.Replace(s, @"[ŒÓŐÔÒȌŎȮȰÖȪŌṒṐÕṌṎȬØǾǪǬỌȎƠ]", "O");
            s = Regex.Replace(s, @"[ṔṖƤⱣ]", "P");
            s = Regex.Replace(s, @"[Ꝗ]", "Q");
            s = Regex.Replace(s, @"[ŔȐŘŖṘṚṜṞɌⱤ]", "R");
            s = Regex.Replace(s, @"[ß§ŚṤŜŠṦŞȘṠṢṨ$]", "S");
            s = Regex.Replace(s, @"[ÞŤṰT̈ŢȚŦṪṬṮȾ]", "T");
            s = Regex.Replace(s, @"[ÚÙÛŬƯŰÜŪṺŲŮŨṸỤṲṴṶɄƲỼ]", "U");
            s = Regex.Replace(s, @"[ṼṾ]", "V");
            s = Regex.Replace(s, @"[ẂŴẀẄW̊ẆẈⱲ₩]", "W");
            s = Regex.Replace(s, @"[X̂ẌẊ]", "X");
            s = Regex.Replace(s, @"[ƳÝŶỲŸȲỸY̊ẎY̨ɎỾ¥]", "Y");
            s = Regex.Replace(s, @"[ŹẐŽŻẒẔƵȤ]", "Z");

            s = Regex.Replace(s, @"[åæāăąàáâãäȁǟą̊ḁẚⱥ@]", "a");
            s = Regex.Replace(s, @"[ḃḅḇƀɓ]", "");
            s = Regex.Replace(s, @"[ćĉčçḉċƈȼ]", "c");
            s = Regex.Replace(s, @"[ɗḓďḋḍḏḑđðɖḏ]", "d");
            s = Regex.Replace(s, @"[éêèȅěëēẽĕęėẹȇḕḗḙḛḝɇ]", "e");
            s = Regex.Replace(s, @"[ḟƒ]", "f");
            s = Regex.Replace(s, @"[ǵĝǧğġģɠḡǥ]", "g");
            s = Regex.Replace(s, @"[ĥȟḧḣḥẖḩḫħⱨ]", "h");
            s = Regex.Replace(s, @"[iıíîìȉïḯīĩįịḭ]", "i");
            s = Regex.Replace(s, @"[ĵǰɉ]", "j");
            s = Regex.Replace(s, @"[ḱǩķḳḵƙⱪ]", "k");
            s = Regex.Replace(s, @"[ĺłḽľļḷḹḻƚɫⱡ]", "l");
            s = Regex.Replace(s, @"[ḿṁṃ]", "m");
            s = Regex.Replace(s, @"[ŋńǹñṋňṅṇṉŉn̈ņ]", "n");
            s = Regex.Replace(s, @"[œóőôòȍŏȯȱöȫōṓṑõṍṏȭøǿǫǭọȏơ]", "o");
            s = Regex.Replace(s, @"[ṕṗƥᵽ]", "p");
            s = Regex.Replace(s, @"[ꝗ]", "q");
            s = Regex.Replace(s, @"[ŕȑřŗṙṛṝṟɍɽ]", "r");
            s = Regex.Replace(s, @"[śṥŝšṧşșṡṣṩ]", "s");
            s = Regex.Replace(s, @"[þťṱẗţțŧṫṭṯⱦ]", "t");
            s = Regex.Replace(s, @"[úùûŭưűüūṻųůũṹụṳṵṷʉµʋỽ]", "u");
            s = Regex.Replace(s, @"[ṽṿ]", "v");
            s = Regex.Replace(s, @"[ẃŵẁẅẘẇẉⱳ]", "w");
            s = Regex.Replace(s, @"[x̂ẍẋ]", "x");
